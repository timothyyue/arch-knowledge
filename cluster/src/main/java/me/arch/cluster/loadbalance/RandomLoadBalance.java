 ...
 */
public class RandomLoadBalance extends AbstractLoadBlance {
    ...
    protected <T extends Resource> T doSelect(List<T> resources) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (sameWeight && i > 0
                    && weight != getWeight(resources.get(i - 1))) {
        ...
        if (totalWeight > 0 && !sameWeight) {
            ...
            // 并确定随机值落在哪个片断上
            for (int i = 0; i < length; i++) {
                offset -= getWeight(resources.get(i));
                if (offset < 0) {
        ...
        // 如果权重相同或权重为0则均等随机
        return resources.get(random.nextInt(length));
    }
}
